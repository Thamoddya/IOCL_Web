...
$('#studentDataTable').DataTable({
    ...
    buttons: ['copy', 'csv', 'excel', {
        extend: 'pdfHtml5', customize: function (doc) {
            ...
            // Add footer text
            doc.content.push({
                text: 'Custom footer text here', margin: [0, 30, 0, 0] // Adjust margin (left, top, right, bottom)
            });
            ...
            doc['header'] = (function () {
                return {
                    columns: [{
                        text: 'Custom Header Text', alignment: 'left', margin: [10, 10]
                    }], margin: 20
                }
            });
        }
...
const OPEN_EditInstructorModel = (instructorId) => {
    $.ajax({
        url: `${BASE_URL}api/getInstructor/${instructorId}`, type: 'POST', success: function (InstructorData) {
...
const DO_UpdateInstructor = () => {
    ...
    Swal.fire({
        ...
        if (isConfirm) {
            $.ajax({
                url: `${BASE_URL}api/updateInstructor`, type: 'POST', data: {
                    ...
                    if (response.message == "success") {
                        ...
                        swal.fire({
                            title: "Error", text: response.message, icon: "error", buttons: false, timer: 1200,
                        });
                    }
                }, error: function (error) {
                    console.log(error);
...
const DO_DeleteInstructor = (instructor_id) => {
    swal.fire({
        ...
        if ((result.isConfirmed)) {
            $.ajax({
                url: `${BASE_URL}api/deleteInstructor`, type: 'POST', data: {
                    "instructor_id": instructor_id
                }, success: function (response) {
                    if (response.message == "success") {
                        swal.fire({
                            title: "Done", text: response.text, icon: "success", buttons: false, timer: 1200,
                        });
                        //response actions...
                        setTimeout(function () {
                            window.location.reload();
...
const DO_RegisterNewInstructor = () => {
    ...
    formData.append('mobile', document.getElementById('REG_InstructorMobile').value);
    formData.append('email', document.getElementById('REG_InstructorEmail').value);
    ...
    formData.append('profile_image', document.getElementById('inputGroupFile01').files[0]);
    ...
    $.ajax({
        ...
        error: function (xhr) {
            ...
            if (errors.email) {
                document.getElementById('error_email').innerText = errors.email[0];
            }
            if (errors.nic) {
            ...
            if (errors.profile_image) {
                document.getElementById('error_profile_image').innerText = errors.profile_image[0];
            }
        }
    });
}
...
const DO_ViewStudent = (IOCL_ID) => {
    $.ajax({
        url: `${BASE_URL}api/getStudent/${IOCL_ID}`, type: 'POST', success: function (StudentData) {
            ...
            $("#STD_Name").text(`${checkData(StudentData.firstName)} ${checkData(StudentData.lastName)}`);
            ...
            if (StudentData.student_details) {
                $("#STD_AddressLineOne").text(checkData(StudentData.student_details.address_line_1));
                $("#STD_AddressLineTwo").text(checkData(StudentData.student_details.address_line_2));
                $("#STD_AddressLineThree").text(checkData(StudentData.student_details.address_line_3));

                // Check if city exists within student_details
                if (StudentData.student_details.city) {
                    $("#STD_City").text(checkData(StudentData.student_details.city.name));
                } else {
                    $("#STD_City").text('N/A');
                }
            } else {
...
const DO_CourseStateChange = (course_no) => {
    $.ajax({
        url: `${BASE_URL}api/course/stateChange`, type: 'POST', data: {
            "course_no": course_no
        }, success: function (response) {
...
function loadCourseDetails() {
    ...
    $.ajax({
        url: `${BASE_URL}api/getCourseDetails/${course_id}`, type: 'POST', success: function (response) {
            ...
            $("#loadButtons").html("");
            $("#loadButtons").html(
                `<button class="btn btn-raised btn-primary" onclick="addCourseVideo(${course_id})">Add Videos</button>
            ...
            response.course.videos.forEach(video => {
                videoHtml += `<div class="timeline-item">
                      <div class="item-content p-2">
                        <div class="panel">
                          <div class="panel-heading p-2" role="tab" id="headingOne_1" style="background-color: #0a58ca">
                                                        <a style="text-decoration: none;font-weight: 600" role="button"
                                                           class="p-2 text-white panel-title h6"
                                                           data-toggle="collapse" href="#collapse${video.id}"
                                                           aria-expanded="true" aria-controls="collapseOne_1">${video.video_title}
                                                        </a>
                                                    </div>
                                                    <div id="collapse${video.id}" class="panel-collapse collapse in"
                                                         role="tabpanel" aria-labelledby="headingOne_1">
                                                        <div class="panel-body p-3">
                                                            <div class="col-12">
                                                                <div class="row">
                                                                    <button onclick="LOAD_VIDEO('${video.video_path}')"
                                                                        class="btn bg-blue-grey col-12 p-1 my-3">
                                                                        LOAD VIDEO
                                                                    </button>
...
function LOAD_VIDEO(video_url) {
    let videoPath = `${BASE_URL}course/WhatsApp Video 2024-05-28 at 18.11.20_5b1ec2ac.mp4`;
    let videoSource = $('#course-video');
    videoSource.attr('src', videoPath);

    // Initialize Video.js player
    let player = videojs('course-video');
